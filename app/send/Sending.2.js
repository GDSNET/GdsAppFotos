...
async function delayedLog(item){
  await delay();
 // Alert.alert(" paso: " + item);

}
  
async function procesessArray(array) {
...
//
  ...
  const json = await AsyncStorage.getItem(nameStorage);
  const newFotos = await JSON.parse(json); 
  await this.setState({ FotosSala: newFotos });
  let progress = 0;
  await this.setState({ indeterminate: false });
  await this.setState({ progress: 0.0 });
  ...
  async function procesessArray(array) {
    array.forEach(async (item) =>{
      ...
      const NewFotosSala = await this.state.FotosSala;
      ...
      const uri = await item.uri;
       await data.append("archivo",{uri: uri,type: "image/jpg",name: "archivo"});
        var  responseDatas =    await  fetch('http://traolcl.gdsnet.com:8500/api/FotosPro/post_GuardaFotos', {
         ...
         await   funexito(responseDatas, uri, filename, newData, id_sala)
  ...
  await this.setState({ exito: false });
   var detalles= await this.state.detalle + " Error en traspaso, " + filename + " - tipo error: " + error.message + " \n"
  await this.setState({
    detalle: detalles
  })
 ...
 async function funexito(responseData, uri, filename, newData, id_sala){
